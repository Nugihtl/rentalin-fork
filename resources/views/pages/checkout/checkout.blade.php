...
<head>
    ...
    <title>{{ isset($isInstallment) ? 'Pembayaran Cicilan' : 'Checkout' }} - Rentalin</title>
...
<body class="bg-[#F5F7FA] text-[#1E1E1E] [font-family:'Plus_Jakarta_Sans',sans-serif]">
    ...
    <main class="max-w-[1100px] mx-auto px-[20px] sm:px-[40px] py-[30px]">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-[22px] sm:text-[26px] font-bold text-gray-900">
                {{ isset($isInstallment) ? 'Pembayaran Cicilan Ke-' . $installment->term_number : 'Checkout' }}
            </h1>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6">
            ...
            <div>
                <div class="bg-white rounded-[10px] border border-[#C3DAFE] p-[20px] sm:p-[24px] shadow-[0px_2px_8px_rgba(15,23,42,0.06)]">
                    ...
                    @if(!isset($isInstallment))
                        @php $deposit = $rental->item->has_deposit ? $rental->item->deposit_amount : 0; @endphp
                    ...
                    @else
                        <div class="flex justify-between text-[13px] text-gray-600 mb-6">
                            <span>Nominal Tagihan Termin {{ $installment->term_number }}</span>
                    ...
                    @else
                        <div class="bg-[#F8FAFC] border border-[#E2E8F0] rounded-[8px] p-[16px] mb-6 flex flex-col items-center">
                            <span class="bg-[#FFF0C2] text-[#D38A00] text-[11px] font-bold px-[10px] py-[4px] rounded-full mb-2">Tagihan Sedang Berjalan</span>
                            <p class="text-[14px] font-bold text-gray-900">Jatuh Tempo: {{ $installment->due_date->format('d M Y') }}</p>
                        </div>
                    @endif
                    ...
                    @if($payment->payment_status != 'paid' || isset($isInstallment))
                    ...
                    @if(isset($payment) && $payment->payment_status == 'expired' && !isset($isInstallment))
    ...
    <script>
        ...
        // FUNGSI INI HANYA BERLAKU UNTUK CHECKOUT PERTAMA KALI
        @if(!isset($isInstallment))
            ...
            function selectPayment(method) {
                ...
                if (method === 'penuh') {
                    ...
                    iconPenuh.classList.add('text-[#34699A]'); iconPenuh.classList.remove('text-gray-400');
                    ...
                    iconPaylater.classList.remove('text-[#34699A]'); iconPaylater.classList.add('text-gray-400');
                    ...
                    iconPenuh.classList.remove('text-[#34699A]'); iconPenuh.classList.add('text-gray-400');
                    ...
                    iconPaylater.classList.remove('text-gray-400'); iconPaylater.classList.add('text-[#34699A]');
        ...
        if (payButton) {
            payButton.addEventListener('click', function() {
                ...
                payButton.innerHTML = 'Memproses Data...';

                // ==========================================
                // JIKA INI ADALAH HALAMAN BAYAR CICILAN
                // ==========================================
                @if(isset($isInstallment))
                    if (!'{{ $snapToken }}') {
                    ...
                    snap.pay('{{ $snapToken }}', {
                        onSuccess: function(result) {
                            ...
                            setTimeout(() => window.location.href = "{{ route('profile.cicilan.show', $payment->id) }}", 1500);
                        },
                        onPending: function(result) {
                            alert('Pembayaran tertunda/menunggu instruksi.');
                            setTimeout(() => window.location.href = "{{ route('profile.cicilan.show', $payment->id) }}", 1500);
                        },
                ...
                // ==========================================
                // JIKA INI ADALAH CHECKOUT TRANSAKSI BARU
                // ==========================================
                @else
                    fetch("{{ route('checkout.process', $rental->id) }}", {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if(data.snap_token) {
                            snap.pay(data.snap_token, {
                                onSuccess: function(result) {
                                    payButton.innerHTML = 'Berhasil';
                                    setTimeout(() => window.location.href = "{{ route('riwayat.transaksi.penyewa') }}", 1500);
                                },
                                onPending: function(result) {
                                    alert('Pembayaran tertunda/menunggu instruksi.');
                                    setTimeout(() => window.location.href = "{{ route('profile.cicilan.index') }}", 1500);
                                },
                                onError: function(result) {
                                    alert('Pembayaran gagal.');
                                    payButton.disabled = false;
