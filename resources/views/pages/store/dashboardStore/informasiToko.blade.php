...
<head>
    ...
    <style>
        ...
        .foto-overlay { position: absolute; inset: 0; border-radius: 50%; background: rgba(0,0,0,0.35); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.2s; pointer-events: none; }
        .foto-wrap:hover .foto-overlay { opacity: 1; }
        .foto-overlay svg { width: 20px; height: 20px; color: #fff; }
        #inputFoto { display: none; }
        .info-input { width: 100%; box-sizing: border-box; border: 1px solid #34699A; border-radius: 8px; padding: 8px 12px; font-size: 14px; font-weight: 500; color: #1E1E1E; font-family: 'Inter', sans-serif; outline: none; box-shadow: 0 0 0 3px rgba(52,105,154,0.1); background: #F9FAFB; display: none; }
        .info-input.textarea { resize: vertical; min-height: 64px; }
        .btn-ubah { background: #fff; color: #374151; font-family: 'Inter', sans-serif; font-size: 13px; font-weight: 500; padding: 8px 20px; border-radius: 8px; border: 1px solid #D1D5DB; cursor: pointer; transition: all 0.2s; text-decoration: none; display: inline-block; white-space: nowrap; flex-shrink: 0; }
        .btn-ubah:hover { border-color: #34699A; color: #34699A; }
        .btn-save { background: #34699A; color: #fff; font-family: 'Inter', sans-serif; font-size: 13px; font-weight: 600; padding: 8px 20px; border-radius: 8px; border: none; cursor: pointer; transition: background 0.2s; display: none; white-space: nowrap; flex-shrink: 0; }
...
<body>
            ...
            <div class="content-card">
                ...
                <div class="info-row">
                    <div class="foto-wrap" onclick="document.getElementById('inputFoto').click()">
                        <img id="fotoPreview"
                             src="{{ $toko->foto_toko ? asset('storage/'.$toko->foto_toko) : asset('assets/img/profile/user-photo-profile.png') }}"
                             alt="Foto Toko"
                ...
                <div class="info-row" id="rowNama">
                    ...
                    <div class="info-body">
                        ...
                        <div class="info-value" id="displayNama">{{ $toko->nama_toko }}</div>
                        <input type="text" id="inputNama" class="info-input" value="{{ $toko->nama_toko }}" placeholder="Masukkan nama toko">
                    </div>
                    <div class="action-group">
                ...
                <div class="info-row" id="rowDeskripsi">
                    ...
                    <div class="info-body">
                        ...
                        <div class="info-value" id="displayDeskripsi">{{ $toko->deskripsi ?? 'Belum ada deskripsi untuk toko ini.' }}</div>
                        <textarea id="inputDeskripsi" class="info-input textarea" placeholder="Masukkan deskripsi toko">{{ $toko->deskripsi }}</textarea>
                    </div>
                    <div class="action-group">
                ...
                <div class="info-row">
                    ...
                    <div class="info-body">
                        ...
                        <div class="info-value"><a href="#">rentalin.com/{{ urlencode(str_replace(' ', '', $toko->nama_toko)) }} ↗</a></div>
                    </div>
                </div>
                ...
                <div class="info-row" style="border-bottom: none;">
                    ...
                    <div class="info-body">
                        ...
                        <div class="info-value">
                            ...
                            @else
                                <span style="color: #D38A00; font-weight: bold;">MENUNGGU VERIFIKASI</span>
                            @endif
                        </div>
                    </div>
    ...
    <script>
        ...
        async function saveFoto() {
            ...
            formData.append('foto_toko', fileInput.files[0]);

            try {
                const response = await fetch("{{ route('store.pengaturan.updateFoto') }}", {
                    method: 'POST',
                ...
                console.error(error);
                showToast('Terjadi kesalahan saat menyimpan foto.');
            }
        }
        ...
        async function saveEdit(field) {
            ...
            if (field === 'Nama') {
                url = "{{ route('store.pengaturan.updateNama') }}";
                payload = { nama_toko: newVal };
            } else if (field === 'Deskripsi') {
                url = "{{ route('store.pengaturan.updateDeskripsi') }}";
                payload = { deskripsi: newVal };
            }
            ...
            try {
                ...
                console.error(error);
                showToast('Gagal menyimpan ' + field + '.');
            }
        }
